...
#load wrist data
    ...
    df = pd.read_csv(os.path.join(file_path, accel_files[0]), engine='python')

    timeArr = df.iloc[:, 0].values
    reliabDf = calc_reliability(timeArr, "second", os.path.join(file_path, accel_files[0]), plot=0)
    ...
    wear_time_file = os.path.join(file_path, 'wear_time_{}.csv'.format(participant))
