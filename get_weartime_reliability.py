...
def df_to_datetime_tz_aware(in_df, column_list, timeUnit='ms'):
...
def separate(watch_df, period_rel_df, time_col, threshold):
    """
    Parameters:
        Required:
    ...
    watch_df = df_to_datetime_tz_aware(watch_df, ['Datetime'], timeUnit='ms')

    period_rel_df['Datetime'] = period_rel_df['Time']
    period_rel_df = df_to_datetime_tz_aware(period_rel_df, ['Datetime'], timeUnit='s')

    sqd = []
    reliabilities = []  # short for reliabilities
    everymin = []
    ...
    while thismin < watch_df['Datetime'].iloc[len(watch_df['Datetime']) - 1]:
        ...
        rel_temp = period_rel_df.loc[
            (period_rel_df['Datetime'] >= thismin) & (period_rel_df['Datetime'] < nextmin)].reset_index(drop=True)
        if len(temp) > 1:
            ...
            reliabilities.append(np.mean(rel_temp['Reliability'].values))
    ...
    return labels, everymin, reliabilities


def get_weartime_reliability(watch_df, period_rel_df, separate, time_col, threshold):
    """
    Parameters:
        Required:
        - watch_df -- raw pandas data frame
    ...
    """
    labels, everymin, reliabilities = separate(watch_df, period_rel_df, time_col, threshold)
    ...
    return everymin, labels_after_rules, reliabilities
...
if __name__ == '__main__':
    ...
    # test_get_weartime_logic()
    wrist_directory = r"D:\HABitsLab\WristDataChecks\output\{}".format("502")
    for dirpath, _, filenames in os.walk(wrist_directory):
        ...
        for csv_file in csv_files:
            ...
            everymin, labels_after_rules, reliability = get_weartime_reliability(df, separate, 'Time', 'ms', 0.005)
            print('total output minutes: ', len(labels_after_rules))
